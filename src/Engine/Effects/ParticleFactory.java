...
public class ParticleFactory 
{
    ...
    public void createExplosion(float x, float y, int numParticles)
    {
        for(int i =0; i <numParticles; i ++)
        {
            ...
            r.color= new Color(100,255,50).getRGB();
            p.renderer = r;
            p.lifeTime = (float)Math.random()*(10-5)+5;
            p.size =(float)Math.random()*(10-5)+5;
            particles.add(p);
        }
    }
    public void createExplosion(float x, float y, int numParticles,int color, Vector2D attract)
    {
        for(int i =0; i <numParticles; i ++)
        {
            Particle p = new Particle();
            ...
            r.color= color;
            p.renderer = r;
            AttractorParticleBehaviour gpb = new AttractorParticleBehaviour(attract);
            p.behaviour = gpb;
            p.lifeTime = (float)Math.random()*(15-10)+10;
            p.size =(float)Math.random()*(10-5)+5;
            particles.add(p);
        }
    }
